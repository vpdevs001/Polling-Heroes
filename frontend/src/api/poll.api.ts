...
export async function createPoll(body: unknown) {
  const { data } = await api.post<
    ApiEnvelope<{ poll: { id: string; url: string } }>
  >("/polls", body);
  return data;
}
...
export async function getMyPolls() {
  const { data } =
    await api.get<ApiEnvelope<{ polls: PollListItem[] }>>("/polls");
  return data;
}

export async function getPoll(pollId: string) {
  const { data } = await api.get<
    ApiEnvelope<{ poll: PollListItem; questions: Question[] }>
  >(`/polls/${pollId}`);
  return data;
}

export async function getPublicPoll(slug: string) {
  const { data } = await api.get<ApiEnvelope<PublicPollPayload>>(
    `/polls/public/${slug}`,
  );
  return data;
}
